...
class InjiClient {
  ...
  constructor() {
    this.isMockMode = !config.features.inji.apiKey || config.features.inji.mockMode;
    this.apiKey = config.features.inji.apiKey || '';
    this.issuerDid = config.features.inji.issuerDid || 'did:example:agriqcert';
    this.webhookSecret = config.features.inji.webhookSecret || '';

    this.httpClient = axios.create({
      baseURL: config.features.inji.apiUrl,
      timeout: 30000,
      headers: {
