...
import type { VerifyVCResponse } from './injiClient.js';
...
class VerifyService {
  ...
  async verify(request: VerifyRequest): Promise<VerifyResult> {
    ...
    try {
      ...
      if (config.inji.apiKey && config.inji.mockMode !== 'true') {
        try {
  ...
  private async parseInput(request: VerifyRequest): Promise<{ vcJson: Record<string, any> | null; vcUrl: string | null }> {
    ...
    if (request.vcJson) {
      ...
      if (config.inji.mockMode === 'true' || config.inji.mockMode === '1') {
  ...
  private verifySignatureLocally(vcJson: Record<string, any>): boolean {
    ...
    if (config.inji.mockMode === 'true' && proof.proofValue && proof.proofValue.includes('mock')) {
      return true;
    }
